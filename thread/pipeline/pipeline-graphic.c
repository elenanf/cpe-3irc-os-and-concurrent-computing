...
pthread_barrier_t barrier_relaunch;
...
static void *triangle_stage(void *a)
{
  ...
  while (triangle_creator_get_next_triangle(triangle_creator, &triangle))
  {
    ...
    if (triangle_is_sync(&triangle))
    {
      ...
      {
        struct Fragment sync_frag = fragment_sync();
        sem_wait(&fragment_sem_prod);
        pthread_mutex_lock(&fragment_mutex);
        fragments[fragment_write_index] = sync_frag;
      ...
      pthread_barrier_wait(&barrier_relaunch);
...
int main(int argc, char *argv[])
{
  ...
  pthread_barrier_init(&barrier_relaunch, NULL, nb_shaders + nb_rasters + 1);
